...
| 16 | Require Rules for Triaged | Triaged | Require Description, Acceptance Criteria, Priority |
...
First run `0D_BUGS_a_setup.md`, then run this entire block in the same
PowerShell session.
...
$requiredStates = @(
    'New', 'Triaged', 'Ready for Dev', 'Dev In Progress', 'Code Review',
...
$logicalRules = @(
    ...
    @{
        name = 'Require Rules for Triaged'; state = 'Triaged'; actions = @(
            @{ type = 'makeRequired'; field = 'Description' }
