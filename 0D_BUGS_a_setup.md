# Azure DevOps API rules: Bug state transitions

These rules restrict the destination states available after an Bug moves
...
# Discover the real Bug reference name. An inherited Bug commonly uses
# Microsoft.VSTS.WorkItemTypes.Bug rather than ScrumHybridGovernance.Bug.
...
${Bug} = $workItemTypes.value | Where-Object { $_.name -eq 'Bug' }

if (-not ${Bug}) {
    $workItemTypes.value | Format-Table name, referenceName, customizationType
    throw 'Bug was not found in the selected process.'
}

$witRefName = ${Bug}.referenceName
Write-Host "Bug reference name: $witRefName"
...
# Define the required states for Bug
$requiredStates = @(
    'New', 
    'Triaged', 
    'Ready for Dev', 
...
if ($missingStates.Count -gt 0) {
    throw "Missing Bug states: $($missingStates -join ', ')"
}
