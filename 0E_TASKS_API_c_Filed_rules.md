# TASKS field-validation API rules

This document creates or updates TASKS field-validation rules 7–12. It resolves
...
initialized for the Scrum Hybrid Governance TASKS. Existing rules with the same
...
with stale `$TASKSFields` or `$fieldReferences` values from a failed run.
...
# Load the fields currently attached to this TASKS work item type.
...
$TASKSFields = @($fieldsResponse.value)
if ($TASKSFields.Count -eq 0) {
    throw "No TASKS fields were returned by '$fieldsUri'."
}

function Resolve-TASKSFieldReference {
    param(
        [Parameter(Mandatory)]
        [string] $DisplayName,
    ...
    if ($matches.Count -eq 0) {
        Write-Host "Available TASKS fields:" -ForegroundColor Yellow
        $AvailableFields |
            Sort-Object name |
            Format-Table name, referenceName, type, required, readOnly
        throw "TASKS field '$DisplayName' was not found."
    }

    if ($matches.Count -gt 1) {
        $matches | Format-Table name, referenceName, type
        throw "Multiple TASKS fields named '$DisplayName' were found."
    }
...
foreach ($fieldName in $requiredFieldNames) {
    $fieldReferences[$fieldName] = Resolve-TASKSFieldReference `
        -DisplayName $fieldName `
        -AvailableFields $TASKSFields
}

Write-Host 'Resolved TASKS field references:' -ForegroundColor Cyan
...
$fieldRuleDefinitions = @(
    ...
    @{
        name    = 'Require Business Case for To Do'
        state   = 'To Do'
        actions = @(
