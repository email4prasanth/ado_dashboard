# Azure DevOps API rules: Task state transitions

These rules restrict the destination states available after an Task moves
...
# Discover the real Task reference name. An inherited Task commonly uses
# Microsoft.VSTS.WorkItemTypes.Task rather than ScrumHybridGovernance.Task.
...
$Task = $workItemTypes.value | Where-Object { $_.name -eq 'Task' }

if (-not $Task) {
    $workItemTypes.value | Format-Table name, referenceName, customizationType
    throw 'Task was not found in the selected process.'
}

$witRefName = $Task.referenceName
Write-Host "Task reference name: $witRefName"
...
if ($missingStates.Count -gt 0) {
    throw "Missing Task states: $($missingStates -join ', ')"
}
